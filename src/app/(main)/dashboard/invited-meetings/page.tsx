...
export default function Dashboard() {
  const { data, isPending, isError, isSuccess, error } = listInvitedMeetings({
    params: {},
  });
  const invitedMeetings = (data?.meetings as Meeting[]) || [];
      ...
      <div className="mx-auto max-w-7xl space-y-8">
        ...
        <div className="space-y-2">
          ...
          <p className="text-muted-foreground text-lg">
            Manage your invited meetings and stay organized
          </p>
        </div>
          ...
          <Card className="border-muted border-2">
            <CardContent className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <p className="text-muted-foreground">
                Please wait while we fetch your invited meetings...
              </p>
            </CardContent>
          </Card>
        )}
          ...
          <>
            ...
            <Card className="border-primary/20 bg-primary/5 border-2">
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-muted-foreground flex items-center space-x-4 text-sm">
                    <div className="flex items-center space-x-1">
                      ...
                      <span>
                        {invitedMeetings.reduce(
                          (acc, m) => acc + m._count.takes,
                          0,
                        )}{" "}
                        total takes
                      </span>
                ...
                <CardDescription>
                  You have {invitedMeetings.length} meeting
                  {invitedMeetings.length !== 1 ? "s" : ""} waiting for you
                </CardDescription>
              </CardHeader>
            </Card>
            ...
            {invitedMeetings.length > 0 ? (
              <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                {invitedMeetings.map((meeting) => (
                  <Link
                    key={meeting.id}
                    href={"invited-meetings/" + meeting.slug}
                  >
                    <Card className="group hover:border-primary/40 border-muted hover:bg-accent/30 gap-0 border-2 transition-all duration-300">
